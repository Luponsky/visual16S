...
#' @param measures The measures to calculate alpha diversity, measures should
#'                 be one of "Observed", "Chao1", "ACE", "Shannon", "Simpson",
#'                 "InvSimpson", "Fisher".
...
plot_alpha_diversity <- function (phyloseq, feature, feature2 = NA, measures,
                                  p_test = "wilcox") {
  set.seed(99)
  ...
  if (p_test == "wilcox") {
    ...
    feature_tab_4_MWtest[[feature]] <-
      plyr::mapvalues(feature_tab_4_MWtest[[feature]], from = replace_feature,
                      to = c(0, 1))
