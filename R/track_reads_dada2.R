#' track_reads_dada2
#'
#' track_reads_dada2 can track the reads count in a dada2 workflow result which
#' created by Xbiome 16S pipeline. Xbiome 16S pipeline dada2 workflow will
#' generate a list that contain sequence table, taxonomy table and reads track
#' data frame. Input the reads track data frame and read type, this function
#' can draw a line plot of reads track of every sample. X-axis will be every
...
track_reads_dada2 <- function(reads_track, single_end = FALSE,
  ...
  reads_track <- rownames_to_column(as.data.frame(reads_track),
                                    var = "SampleID")
  reads_track$SampleID <- factor(reads_track$SampleID)
  reads_track <- gather(reads_track, 2:ncol(reads_track), key = "stages",
                        value = "reads")
  if (single_end) {
    reads_track$stages <- factor(reads_track$stages,
